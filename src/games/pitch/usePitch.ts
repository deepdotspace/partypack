/**
 * usePitch — thin typed wrapper over the route-owned game-room connection.
 *
 ...
 *
 * Room binding: JOIN always carries `game: 'pitch'` — on a fresh (unbound)
...
export interface Pitch {
  ...
  send: {
    ...
    start: () => void
    skip: () => void
    submit: (name: string, pitch: string) => void
...
export function usePitch(room: RoomApi, code: string, role: ClientRole): Pitch {
  ...
  const state: GameState | null =
    room.state &&
    (room.state as { game?: unknown }).game === 'pitch' &&
  ...
  const send = useMemo(
    () => ({
      join: (name: string) => {
        storeName(name)
        room.sendInput('JOIN', { name, cid, color: readStoredColor(), roomCode: code, game: 'pitch' })
      },
      setColor: (color: string) => {
        storeColor(color)
        room.sendInput('JOIN', { name: readStoredName(), cid, color, roomCode: code, game: 'pitch' })
      },
      ...
      submit: (name: string, pitch: string) => room.sendInput('SUBMIT', { name, pitch }),
  ...
  useEffect(() => {
    ...
    if (role === 'play') {
      ...
      if (name) room.sendInput('JOIN', { name, cid, color: readStoredColor(), roomCode: code, game: 'pitch' })
    }
